...
view.lessonAccordion = () => {
  ...
  for (i = 0; i < curriculumItems.length; i++) {
    ...
    let firstLevel = curriculumItems[i].querySelectorAll(".section-list > li")
    ...
    for (j = 0; j < firstLevel.length; j++) {
      if (firstLevel[j].classList.contains("section-link")) {
        let sectionTitle = firstLevel[j].querySelector(".section-link a")
        let sectionList = firstLevel[j].querySelector(".section-link ul")
        let secondLevel = firstLevel[j].querySelectorAll(".section-link ul li")
